...
public class ChefImpl implements Chef, BeltObserver {
	...
	public void makeAndPlacePlate(Plate plate, int position) 
		...
		if (plate.getContents().getName().equals("")) {
			throw new RuntimeException("At least this isn't a null pointer exception");
		}
		if (plate.getContents().getIngredients().length == 0) {
			throw new RuntimeException("get some food");
		}
	...
	public void handleBeltEvent(BeltEvent e) {
		if (e.getType() == BeltEvent.EventType.PLATE_CONSUMED) {
			...
			if (plate.getChef() == this) {
				...
				IngredientPortion[] portions = plate.getContents().getIngredients();
				for (int i = 0; i < portions.length; i++) {
					consumedAmount += portions[i].getAmount();
				}
			...
			if (plate.getChef() == this) {
				for (int i = 0; i < portions.length; i++) {
					spoiledAmount += portions[i].getAmount();
				}
